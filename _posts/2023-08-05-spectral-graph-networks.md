...
Thus, $$\mathbf{x}^T \mathbf{L} \mathbf{x}$$ is taking the squared difference between each pair of elements of $$\mathbf{x}$$ that are directly connected in the graph, then summing across all the pairs. Thus, a low $$\mathbf{x}^T \mathbf{L} \mathbf{x}$$ means $$\mathbf{x}$$ varies smoothly across the graph, whereas a high $$\mathbf{x}^T \mathbf{L} \mathbf{x}$$ means $$\mathbf{x}$$ varies quickly across the graph.
